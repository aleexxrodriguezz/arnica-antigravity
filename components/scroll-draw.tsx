...
import { useSoundEnabled, useParallaxTechSound } from '@/hooks/use-sounds'
import { useIsMobile } from '@/hooks/use-mobile'

/* ─── Theme-aware color helpers ─────────────────────────────────────
   Light mode (day): black lines
...
const getPaths = (colors: ReturnType<typeof getColors>) => [
  ...
  // ── Diagonal connectors (upper-left → ring)
  ...
  // ── Diagonal connectors (upper-right → ring)
  ...
  // ── Corner node detailing — top-left
  ...
  // ── Corner node detailing — top-right
  ...
  // ── Corner node detailing — bottom-left
  ...
  // ── Corner node detailing — bottom-right
  ...
  */
  ...
  return (
    <section
      ...
      className="relative"
      aria-label="Scroll-driven diagram animation"
    >
      {/* ── Scroll distance container: 500vh ── */}
      <div style={{ height: '500vh' }}>
        {/* ── Sticky viewport ── */}
        <div className="sticky top-0 h-screen overflow-hidden flex items-center justify-center">

          {/* Hero title visible at start */}
          <motion.div
            className="absolute top-20 left-0 right-0 text-center z-30 px-6"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: progress < 0.15 ? 1 : 0, y: progress < 0.15 ? 0 : -20 }}
            transition={{ duration: 0.6 }}
          >
          </motion.div>

          {/* Ambient glow - white tint in dark mode */}
          <div
          ...
          <motion.div
            ...
            style={{ y: svgY }}
          >
            {/* Section label with sound toggle */}
            <motion.div
            ...
            >
              ...
              {/* Node dots */}
              {NODES.map((n, i) => {
                const visible = progress >= n.triggerAt
                return (
                  <g key={i}>
                    <circle
                      cx={n.cx}
              ...
              {/* Pulse dot on centre node when progress > 0.35 */}
              {progress > 0.35 && (
          ...
          >
            <motion.div
              className="w-6 h-10 rounded-full border-2 border-foreground/50 flex justify-center pt-2"
              initial={{ opacity: 0.8 }}
            >
              <motion.div
